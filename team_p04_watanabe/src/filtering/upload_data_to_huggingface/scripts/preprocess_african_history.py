...
def preprocess_african_history_qa(example):
    """
    African-History-Extra-Dspy-QA-Reasoningデータセットを前処理し、
...
def main():
    # --- Configuration ---
    SOURCE_DATASET_ID = "Svngoku/African-History-Extra-Dspy-QA-Reasoning"
    ...
    try:
        processed_dataset_dict.push_to_hub(
            repo_id=TARGET_DATASET_ID,
            private=False,
            commit_message=f"Preprocessed {SOURCE_DATASET_ID} for LLM training (no #### tag in answer)"
        )
        print("\nUpload successful!")
