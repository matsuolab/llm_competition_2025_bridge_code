import datasets
from huggingface_hub import login, create_repo

def preprocess_african_history_qa_with_filter(example):
    """
    African-History-Extra-11-30-24-QA-Pairs-Reasoningデータセットを前処理し、
    questionとanswerの形式に変換します。
    reasoningを<think>タグで囲み、answerと結合します。
    answerには####タグを含めません。
    """
    question_content = example.get('question', '')
    reasoning_content = example.get('reasoning', '')
    answer_content = example.get('answer', '')

    # reasoningを<think>タグで囲む
    # reasoningがない場合は<think></think>とする
    if reasoning_content:
        think_tag_content = f"<think>{reasoning_content}</think>"
    else:
        think_tag_content = "<think></think>"

    # answer形式: <think>{思考プロセス}</think>\n\n{最終的な答え} (####タグなし)
    formatted_answer = f"{think_tag_content}\n\n{answer_content}"

    return {
        "question": question_content,
        "answer": formatted_answer
    }

def filter_by_confidence_score(example):
    """
    confidence_scoreが0.7以上の例のみをフィルタリングします。
    """
    confidence_score = example.get('confidence_score')
    return confidence_score is not None and confidence_score >= 0.7

def main():
    # --- Configuration ---
    SOURCE_DATASET_ID = "Svngoku/African-History-Extra-11-30-24-QA-Pairs-Reasoning"
    TARGET_DATASET_ID = f"{SOURCE_DATASET_ID.replace('/', '_')}_preprocess"

    print(f"Loading dataset: {SOURCE_DATASET_ID}")
    dataset = datasets.load_dataset(SOURCE_DATASET_ID)

    print("Transforming dataset...")
    processed_splits = {}
    for split_name, ds_split in dataset.items():
        print(f"Processing split: {split_name}")
        
        # フィルタリング
        filtered_split = ds_split.filter(filter_by_confidence_score)
        print(f"Original {split_name} size: {len(ds_split)}")
        print(f"Filtered {split_name} size: {len(filtered_split)}")

        # 変換
        processed_splits[split_name] = filtered_split.map(
            preprocess_african_history_qa_with_filter,
            remove_columns=filtered_split.column_names,
            num_proc=4 # 並列処理
        )
        print(f"Processed {split_name} size (after filter and map): {len(processed_splits[split_name])}")

    processed_dataset_dict = datasets.DatasetDict(processed_splits)
    print("Transformation complete.")

    # --- Hugging Face Hub Upload ---
    print("\nPlease ensure you are logged into the Hugging Face Hub with write access.")
    print("If not, run 'huggingface-cli login' in your terminal and enter your token.")
    
    try:
        print(f"Ensuring repository exists on the Hub: {TARGET_DATASET_ID}")
        create_repo(
            repo_id=TARGET_DATASET_ID,
            repo_type="dataset",
            private=False,
            exist_ok=True
        )
        print(f"Repository '{TARGET_DATASET_ID}' created or already exists.")
    except Exception as e:
        print(f"Error creating or checking repository: {e}")
        print("Please ensure you have write access to the Hugging Face Hub.")
        return

    print(f"Uploading processed dataset to Hugging Face Hub: {TARGET_DATASET_ID}")
    try:
        processed_dataset_dict.push_to_hub(
            repo_id=TARGET_DATASET_ID,
            private=False,
            commit_message=f"Preprocessed {SOURCE_DATASET_ID} for LLM training (filtered by confidence_score >= 0.7)"
        )
        print("\nUpload successful!")
        print(f"Dataset available at: https://huggingface.co/datasets/{TARGET_DATASET_ID}")
    except Exception as e:
        print(f"\nAn error occurred during upload: {e}")
        print("Please ensure you are logged into the Hugging Face Hub with write access.")
        print("Run the following command in your terminal and enter your token:\n  huggingface-cli login")

if __name__ == "__main__":
    main()
