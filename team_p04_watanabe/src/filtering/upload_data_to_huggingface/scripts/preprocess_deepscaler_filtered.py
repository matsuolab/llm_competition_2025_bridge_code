...
def is_solution_not_null(example):
    """
    Returns True if the 'solution' field is not None and not an empty string.
    """
    solution = example.get('solution')
    return solution is not None and str(solution).strip() != ''
...
def main():
    """
    Main function to load, filter, process, and upload the DeepScaleR dataset.
    """
    # --- Configuration ---
    SOURCE_REPO = "agentica-org/DeepScaleR-Preview-Dataset"
    TARGET_REPO = "daichira/DeepScaleR-Preview-Dataset_preprocess_filtered"
    TEST_SIZE = 0.01
    RANDOM_SEED = 42
    ...
    # --- Filtering ---
    print("Filtering out examples where 'solution' is null or empty...")
    filtered_dataset = source_dataset.filter(is_solution_not_null, num_proc=num_proc)
    print(f"Size after filtering: {len(filtered_dataset)}")

    # --- Transformation ---
    print("Transforming filtered dataset...")
