#!/usr/bin/env python3
"""
Complete HLE Dataset Analysis Pipeline
=====================================

This script provides a comprehensive analysis pipeline for the HLE (Humanity's Last Exam) dataset.
It includes data loading, statistical analysis, visualization, and embedding-based analysis.

Usage:
    python hle_complete_analysis.py [--samples 500] [--embeddings] [--visualizations] [--all]
"""
...
def setup_output_directory():
    ...
    output_dir = Path("analysis_outputs/hle_analysis_output")
...
### Embedding Analysis
```python
from hle_embeddings import HLEEmbeddingAnalyzer
analyzer = HLEEmbeddingAnalyzer(use_local_model=True)
...
def run_comprehensive_embedding_analysis_with_path(df, output_dir, save_embeddings=True, dataset_name="hle"):
    ...
    try:
        ...
        analyzer = HLEEmbeddingAnalyzer(use_local_model=use_local_model)
...
def main():
    ...
    parser.add_argument(
        '--dataset', type=str, help='Dataset to analyze: either a HuggingFace dataset name or path to a JSONL file'
    )
    parser.add_argument(
