...
# Store the script start time for data_id generation
SCRIPT_START_TIME = datetime.now()
timestamp = SCRIPT_START_TIME.strftime("%Y%m%d_%H%M%S")
folder_name = f"generated_{timestamp}"

# Use the specified output directory - match generate_data.py output location
...
# Subject and question type distributions from generate_data.py
...
# Number of problems to generate per seed
...
def initialize_vllm():
    ...
    global llm_vllm, sampling_params_solver, sampling_params_judge, USE_VLLM
...
problem_cleaner_prompt = ChatPromptTemplate.from_messages([
...
genselect_prompt = ChatPromptTemplate.from_messages([
    SystemMessagePromptTemplate.from_template(
...
def vllm_generate(prompt: Union[str, Any], sampling_params: SamplingParams) -> str:
...
if USE_VLLM:
    # vLLM mode
    problem_cleaner_chain: Runnable = (
        problem_cleaner_prompt
    ...
    problem_generator_chain: Runnable = (
        problem_generator_from_seed_prompt
        | RunnableLambda(lambda prompt_value: vllm_generate(
            prompt_value,
            sampling_params_solver
        ))
    ...
    # API mode - add generator LLM
    llm_generator = ChatOpenAI(
        model=MODEL_NAME_SOLVER, 
        temperature=0.8,
        base_url="https://api.deepseek.com"
    )
    llm_cleaner = ChatOpenAI(
        model=MODEL_NAME_SOLVER,
        temperature=0.3,
        base_url="https://api.deepseek.com"
    )
    problem_cleaner_chain: Runnable = problem_cleaner_prompt | llm_cleaner | StrOutputParser()
    problem_generator_chain: Runnable = problem_generator_from_seed_prompt | llm_generator | StrOutputParser()
    cot_solver_chain: Runnable = cot_solver_prompt | llm_solver | StrOutputParser()
    genselect_chain: Runnable = genselect_prompt | llm_judge | StrOutputParser()
...
def generate_new_data_id(seed_data_id: str, generation_index: int, template_name: str = "origin", generation_timestamp: Optional[datetime] = None) -> str:
    """Generate new data_id for synthetic data.
    
    Input: (source_name)_(subject)_(7digits)_seed_(timestamp)_000
    Output: (source_name)_(subject)_(7digits)_sft_(YYYYMMDDhhmmss)_(generation_index)_(template)
    """
    try:
        ...
        # Build new data_id
        new_data_id = '_'.join(base_parts) + f'_sft_{timestamp}_{generation_index:03d}_{template_name}'
        return new_data_id
    except Exception as e:
        print(f"Error generating data_id: {e}")
        # Fallback data_id
        timestamp_to_use = generation_timestamp if generation_timestamp else SCRIPT_START_TIME
        return f"unknown_sft_{timestamp_to_use.strftime('%Y%m%d%H%M%S')}_{generation_index:03d}_{template_name}"
...
async def generate_problem_from_seed(seed_problem: str, subject: str, question_type: str) -> str:
    ...
    try:
        ...
        response = await problem_generator_chain.ainvoke({
...
async def generate_multiple_cots(question: str, num_candidates: int = NUM_COT_CANDIDATES) -> List[Dict[str, str]]:
    ...
    if USE_VLLM:
        # Batch generation for vLLM - much faster than sequential
        try:
            ...
            for i in range(num_candidates):
                try:
                    result = await cot_solver_chain.ainvoke({"problem": question})
                    # Parse the CoT to extract think and answer
                    think, answer = parse_cot_output(result, detect_question_type(question))
        ...
        # Batch generation for API mode
        batch_size = 4
        for i in range(0, num_candidates, batch_size):
            batch_end = min(i + batch_size, num_candidates)
            batch_tasks = []
            
            for j in range(i, batch_end):
                task = cot_solver_chain.ainvoke({"problem": question})
            ...
            for j, result in enumerate(batch_results):
                if isinstance(result, Exception):
                    print(f"  Error generating CoT {i+j+1}: {result}")
                    continue
                ...
                print(f"  Generated CoT {i+j+1}/{num_candidates}, Answer: {answer}")
...
async def select_best_cot(question: str, cot_results: List[Dict[str, str]], 
    ...
    try:
        selection = await genselect_chain.ainvoke({
            "problem": question,
...
async def clean_seed_problem(seed_problem: str, question_type: str) -> str:
    ...
    try:
        response = await problem_cleaner_chain.ainvoke({
    ...
    for prob_num in range(PROBLEMS_PER_SEED):
        try:
            ...
            if prob_num == 0:
                ...
                generated_item = {
                    "data_id": generate_new_data_id(seed_data_id, prob_num, "origin", generation_timestamp),
...
def append_single_item(item: Dict[str, Any], save_dir: str):
    ...
    for template_name, formatted_item in template_results.items():
        # Update data_id with the correct template name
        if 'data_id' in cleaned_item:
            # Replace the template name part in data_id
            old_data_id = cleaned_item['data_id']
            parts = old_data_id.rsplit('_', 1)  # Split from the right to get template part
            if len(parts) == 2:
                new_data_id = f"{parts[0]}_{template_name}"
                formatted_item['data_id'] = new_data_id
        
        template_filepath = os.path.join(save_dir, f"enhanced_seed_dataset_{template_name}.jsonl")
...
async def process_seed_dataset(dataset_name: str = "team-suzuki/SEED_000_origin_v2", output_dir: str = None, limit: int = None, resume: bool = False):
