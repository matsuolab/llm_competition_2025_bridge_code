...
print(f"Environ OPENAI_API_KEY: {openai_key_env}")
print(f"Environ GEMINI_API_KEY: {gemini_key_env}")

# 2. info.jsonを読み込んでいるか確認 (info_dataが利用可能か)
try:
    # 親ディレクトリをパスに追加
    ...
    openai_key_info = info_data.get("OpenAI")
    gemini_key_info = info_data.get("Gemini")
    print(f"info.json OpenAI key: {openai_key_info}")
    print(f"info.json Gemini key: {gemini_key_info}")
except Exception as e:
    print(f"Could not import or read from info_data: {e}")
print("="*50)
# ===== ここまでデバッグコード =====
