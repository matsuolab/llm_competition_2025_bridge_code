#!/usr/bin/env python3
"""
Data selector for ReasonMed dataset with half-gaussian distribution favoring longer outputs.
Based on length_selector.py but adapted for ReasonMed dataset structure.
"""
...
class ReasonMDLengthSelector:
    """Length-based selector for ReasonMD dataset favoring longer outputs."""
    
    def __init__(self, target_samples=1000, sample_size_for_stats=5000, num_bins=6, curve_sharpness=3.0):
    ...
    def _calculate_bucket_sizes(self):
        ...
        min_samples_per_bucket = max(20, self.target_samples // (num_buckets * 3))
...
def select_reasonmd_data(target_samples=1000, sample_size_for_stats=5000, num_bins=6, curve_sharpness=3.0, hf_token=None):
    """Select data from ReasonMD dataset with half-gaussian distribution."""
    print(f"Loading ReasonMD dataset...", file=sys.stderr)
    
    dataset = load_dataset("lingshu-medical-mllm/ReasonMed", streaming=True, token=hf_token)
    train_dataset = dataset['train']
    
    # Initialize selector
    selector = ReasonMDLengthSelector(target_samples, sample_size_for_stats, num_bins, curve_sharpness)
    ...
    for item_idx, item in enumerate(train_dataset):
        ...
        if item_idx > 20000 and len(selector.get_selected_items()) >= target_samples:
...
def print_length_stats(items):
    ...
    lengths = [len(str(item['output'])) for item in items]
...
def main():
    ...
    # Select data
    selected_items = select_reasonmd_data(
