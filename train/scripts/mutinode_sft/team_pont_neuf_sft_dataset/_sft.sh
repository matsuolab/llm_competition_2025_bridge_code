...
#SBATCH -p P06
#SBATCH --nodelist=osk-gpu[66,68,69,70,71,73]
#SBATCH --nodes=6
...
MASTER_PORT=37171 # not used in sft.sh
...
for node in "${NODES[@]}"; do
    ...
    ssh -q $node "
        cd $SCRIPT_ROOT && \
        bash $SCRIPT_ROOT/scripts/mutinode_sft/team_pont_neuf_sft_dataset/sft.sh $MASTER_ADDR $MASTER_PORT $NODE_RANK $NNODES $GPUS_PER_NODE
