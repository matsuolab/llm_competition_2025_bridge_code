#!/bin/bash
#SBATCH --job-name=deepseek_r1_0528_peft_8gpu
...
for node in "${NODES[@]}"; do
    ...
    ssh -q $node "
        cd $SCRIPT_ROOT && \
        bash $SCRIPT_ROOT/scripts/mutinode_sft/run_deepseek_r1_0528_peft.sh $MASTER_ADDR $MASTER_PORT $NODE_RANK $NNODES $GPUS_PER_NODE
