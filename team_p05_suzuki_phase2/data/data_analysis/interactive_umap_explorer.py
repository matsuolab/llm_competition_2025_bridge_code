...
class InteractiveUMAPExplorer:
    ...
    def create_interactive_umap(
            ...
            for i, value in enumerate(unique_values):
                ...
                if 'dataset' in self.df.columns:
                    for idx in indices:
                        ...
                        if str(dataset_name).upper().startswith('HLE'):
        ...
        if 'dataset' in self.df.columns:
            ...
            has_hle = any(str(dataset).upper().startswith('HLE') for dataset in self.df['dataset'])
            has_non_hle = any(not str(dataset).upper().startswith('HLE') for dataset in self.df['dataset'])
