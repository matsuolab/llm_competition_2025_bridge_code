...
#SBATCH --job-name=generate_data_rag
...
readonly SCRIPT_DIR="/home/Competition2025/P05/$USER/team_suzuki/data/data_generation/sft_data_generation/v3_seed_with_rag"
...
export USE_RAG=true  # RAGを有効化
export USE_VLLM=true
export VECTOR_DB_PATH="${SHARED_DATA_DIR}/vector_store/rag_index.faiss"
export EMBEDDING_MODEL="sentence-transformers/all-MiniLM-L6-v2"  # より軽量なモデル
export GPU_MEMORY_UTILIZATION=0.80  # メモリ使用量をさらに削減
export RAG_TOP_K=5  # 高品質な候補を優先
export RAG_SIMILARITY_THRESHOLD=0.5  # バランスの取れた閾値（品質を保ちつつ適切な取得を実現）
export INCLUDE_SOLUTIONS_IN_EVIDENCE=true
export REQUIRE_CITATIONS=true
export ADAPTIVE_RETRIEVAL=true
...
mkdir -p ${SHARED_DATA_DIR}/v3_seed_with_rag
mkdir -p ${SHARED_DATA_DIR}/vector_store
# 新規作成したディレクトリのみ権限設定（既存ファイルは除外）
chmod 777 ${SHARED_DATA_DIR}/logs 2>/dev/null || true
chmod 777 ${SHARED_DATA_DIR}/v3_seed_with_rag 2>/dev/null || true
chmod 777 ${SHARED_DATA_DIR}/vector_store 2>/dev/null || true
...
# Miniconda環境のアクティベート
echo "Starting RAG-enhanced data generation at $(date)"
...
# Install additional dependencies for RAG if needed
echo "Checking and installing RAG dependencies..."
pip install -q faiss-cpu sentence-transformers 2>/dev/null || echo "RAG dependencies already installed"
...
OUTPUT_DIR=""
DATASET="team-suzuki/SEED_000_origin_0813"  # Default dataset
BUILD_INDEX=""
...
while [[ $# -gt 0 ]]; do
    case $1 in
        ...
        *)
            ...
            echo "Usage: sbatch run_generate_data3.sh [options]"
            echo "Options:"
            echo "  --limit N                Number of seeds to process"
            echo "  --problems-per-seed N    Problems to generate per seed"
            echo "  --output-dir DIR         Output directory"
            echo "  --dataset DATASET        Hugging Face dataset name"
            echo "  --build-index           Build RAG index before generation"
            echo "  --no-rag                Disable RAG functionality"
            exit 1
            ;;
    esac
done
...
LOG_FILE="${SHARED_DATA_DIR}/logs/generation_rag_${SLURM_JOB_ID:-local}.log"

echo "=== Starting RAG-enhanced problem generation ==="
echo "Dataset: $DATASET"
echo "Limit: $LIMIT seeds"
echo "Problems per seed: $PROBLEMS_PER_SEED"
echo "Output directory: ${OUTPUT_DIR:-auto-generated}"
echo "RAG enabled: $USE_RAG"
if [ "$USE_RAG" = "true" ]; then
    echo "Build index: ${BUILD_INDEX:-no}"
...
# Use the generate_data script directly without wrapper
GENERATE_SCRIPT="${SCRIPT_DIR}/generate_data.py"

# Build command without wrapper
CMD="python -u $GENERATE_SCRIPT --limit $LIMIT --problems-per-seed $PROBLEMS_PER_SEED --dataset $DATASET $BUILD_INDEX"

if [ -n "$OUTPUT_DIR" ]; then
    CMD="$CMD --output-dir $OUTPUT_DIR"
else
    # デフォルトの出力先を共有データディレクトリに設定
    DEFAULT_OUTPUT_DIR="${SHARED_DATA_DIR}/v3_seed_with_rag/$(date +%Y%m%d_%H%M%S)"
...
if [ "${SLURM_NODEID:-0}" -eq 0 ]; then
    if [ $PYTHON_EXIT_CODE -eq 0 ]; then
        ...
        # Show RAG statistics if enabled
        if [ "$USE_RAG" = "true" ] && [ -f "$VECTOR_DB_PATH" ]; then
            echo "=== RAG Vector Store Statistics ==="
            echo "Index size: $(du -sh "$VECTOR_DB_PATH" 2>/dev/null | cut -f1)"
        fi
    else
