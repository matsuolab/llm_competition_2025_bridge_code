...
if VLLM_AVAILABLE:
    ...
    raise RuntimeError("vLLM is not available. This refactored version requires vLLM.")
...
DTYPE = os.getenv("DTYPE", "bfloat16")  # Options: "float16", "bfloat16", "float32"
TRUST_REMOTE_CODE = os.getenv("TRUST_REMOTE_CODE", "true").lower() == "true"  # For custom model code
MAX_MODEL_LEN = int(os.getenv("MAX_MODEL_LEN", "16384"))  # Max model length
...
def initialize_vllm():
    ...
    global llm_vllm, sampling_params_generator, sampling_params_solver
...
problem_generator_prompt = SimplePromptTemplate(
    system_template=(
    ...
    human_template="Generate one {difficulty}‑level {subject} {question_type} question now."
)
...
cot_solver_prompt = SimplePromptTemplate(
    ...
    human_template="Problem: {problem}\n"  # 問題文を入力として受け取る
)


# vLLM wrapper function
def vllm_generate(messages: List[Dict[str, str]], sampling_params: SamplingParams, lora_request: Optional[str] = None) -> str:
...
def generate_problem(difficulty: str, subject: str, question_type: str) -> str:
    """Generate a problem using vLLM."""
    messages = problem_generator_prompt.format_messages(
...
def solve_problem(problem: str) -> str:
    """Solve a problem using vLLM."""
    messages = cot_solver_prompt.format_messages(problem=problem)
    return vllm_generate(
        messages,
...
async def generate_single_item(iteration: int, difficulty: str, subject: str, question_type: str, max_retries: int = 3) -> Dict[str, Any]:
    """Generate a single question-think-answer triplet with retry logic.
    ...
    for retry_count in range(max_retries):
        try:
            ...
            # Generate problem using vLLM
            problem = generate_problem(difficulty, subject, question_type)
            print(f"Generated Question:\n{problem}\n")

            cot = solve_problem(problem)
...
def save_data(dataset: List[Dict[str, Any]], save_dir: str):
    """Save complete dataset in JSONL format."""
    os.makedirs(save_dir, exist_ok=True)
...
async def run_generation_pipeline(output_dir=None):
    ...
    if not initialize_vllm():
        ...
        print("ERROR: Cannot use vLLM mode without GPU. Please ensure vLLM is properly configured.")
        return None
    
    print("Starting data generation pipeline...")
    print(f"Backend: vLLM")
    if USE_LOCAL_MODEL:
        print(f"Model: {MODEL_PATH} (local)")
    else:
