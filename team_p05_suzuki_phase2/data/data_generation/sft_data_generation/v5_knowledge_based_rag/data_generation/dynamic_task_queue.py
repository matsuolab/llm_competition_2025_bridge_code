...
File-based task queue for distributed processing with fault tolerance
"""
...
class DynamicTaskQueue:
    """
    Dynamic task queue that manages tasks and handles node addition/removal
    File-based implementation to avoid external DB dependencies
    """
    
    def __init__(self, queue_dir: str = None):
        """
        Args:
            queue_dir: Directory to manage task queue files
        """
        ...
        self.heartbeat_interval = 30  # Heartbeat every 30 seconds
        self.task_timeout = 600  # Task timeout after 10 minutes
                ...
                if task['status'] == 'pending':
                    ...
                    if task['started_at']:
                        ...
                        if elapsed > timeout_threshold:
                            # 異なるワーカーのタスクのみ再割り当て可能
                            if task.get('worker_id') != self.worker_id:
                                logger.warning(f"Task {task['task_id']} timed out after {elapsed:.0f}s (worker: {task.get('worker_id', 'unknown')}), reassigning")
                                task['status'] = 'pending'
