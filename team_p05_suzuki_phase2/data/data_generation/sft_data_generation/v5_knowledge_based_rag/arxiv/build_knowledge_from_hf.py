...
class HuggingFaceKnowledgeBuilder:
    ...
    def load_from_huggingface(self) -> List[Dict]:
        ...
        try:
            ...
            # Load dataset with authentication, handling column mismatches
            try:
                ...
                # If column mismatch occurs, try loading with streaming mode
                logger.warning(f"Column mismatch detected, trying streaming mode: {column_error}")
                ...
                # Collect all data with unified schema
                papers = []
                for idx, item in enumerate(dataset_stream):
                    # Normalize the item to have consistent columns
                    normalized_item = dict(item)
                    # Add missing columns with default values if needed
                    if 'citation_count' not in normalized_item:
                        normalized_item['citation_count'] = 0
                    papers.append(normalized_item)

                    # Log progress every 1000 items
                    if (idx + 1) % 1000 == 0:
                        logger.info(f"Loaded {idx + 1} entries...")

                logger.info(f"✅ Loaded {len(papers)} entries from Hugging Face (streaming mode)")
                return papers
