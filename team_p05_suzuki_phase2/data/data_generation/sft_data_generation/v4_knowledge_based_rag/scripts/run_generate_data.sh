...
#SBATCH --job-name=generate_data_v4_knowledge
...
# Set absolute paths
readonly SCRIPT_DIR="/home/Competition2025/P05/$USER/team_suzuki/data/data_generation/sft_data_generation/v4_knowledge_based_rag"
...
readonly KNOWLEDGE_BASE_DIR="/home/Competition2025/P05/shareP05/knowledge_indexes"
...
mkdir -p ${SHARED_DATA_DIR}/v4_knowledge_based
...
if [ -f "$KNOWLEDGE_INDEX_PATH" ]; then
    echo "✓ Knowledge index found: $KNOWLEDGE_INDEX_PATH"
    echo "  Size: $(du -sh "$KNOWLEDGE_INDEX_PATH" | cut -f1)"
else
    echo "✗ Knowledge index not found: $KNOWLEDGE_INDEX_PATH"
    echo "  Please build the knowledge index first"
fi

if [ -f "$KNOWLEDGE_METADATA_PATH" ]; then
    echo "✓ Knowledge metadata found: $KNOWLEDGE_METADATA_PATH"
    if command -v jq &> /dev/null; then
        echo "  Total documents: $(jq '.total_documents' "$KNOWLEDGE_METADATA_PATH")"
    fi
else
    echo "✗ Knowledge metadata not found: $KNOWLEDGE_METADATA_PATH"
fi
echo ""
...
# Miniconda環境のアクティベート
echo "Starting v4 knowledge-based data generation at $(date)"
...
# ログファイルパス
LOG_FILE="${SHARED_DATA_DIR}/logs/generation_v4_${SLURM_JOB_ID:-local}.log"

echo "=== Starting v4 Knowledge-based Problem Generation ==="
...
# Build command
CMD="python -u ${SCRIPT_DIR}/data_generation/generate_data.py --limit $LIMIT --problems-per-seed $PROBLEMS_PER_SEED --dataset $DATASET $NO_KNOWLEDGE_RAG $EXECUTION_FLAGS"

if [ -n "$OUTPUT_DIR" ]; then
    CMD="$CMD --output-dir $OUTPUT_DIR"
else
    # デフォルトの出力先
    DEFAULT_OUTPUT_DIR="${SHARED_DATA_DIR}/v4_knowledge_based/$(date +%Y%m%d_%H%M%S)"
