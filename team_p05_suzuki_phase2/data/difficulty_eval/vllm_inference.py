...
def main():
    ...
    round_id = sys.argv[2]
    json_name='results-tmp1-Round'+str(round_id)+'-'+dataset_name.split("/")[-1] + '-' + model_name.split("/")[-1] + '.jsonl'
    json_path=os.path.join(currdir,config['output']['dir'],json_name)
    log_name='log-tmp1-Round'+str(round_id)+'-'+ dataset_name.split("/")[-1] + '-' + model_name.split("/")[-1] + '.jsonl'
    ...
    kv_quant = (config['model'].get("kv_cache_quant") or "").strip()
    if kv_quant:
    ...
    sp = SamplingParams(
        ...
        temperature= config['inference']['temparature'],  # set >0 only if you want variability
        top_p=1.0,
    )
    ...
    for batch_id, batch_prompts in enumerate(chunks(prompts, batch_size), start=start_id):
        ...
        subset['model_output']=model_output
