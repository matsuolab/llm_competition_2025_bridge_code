...
class AuditLogger:
    ...
    def log_error(self, sample_id: str, error_msg: str, error_type: str = "unknown", extra_info: Optional[Dict[str, Any]] = None):
        """
        エラーログ記録
        ...
        # 追加情報がある場合は含める
        if extra_info:
            error_entry["extra_info"] = extra_info
...
class ProgressTracker:
    ...
    def __init__(self, total: int, desc: str = "Processing", update_interval: int = 100):
        """
        Args:
            total: 総件数
            desc: 説明文
            update_interval: 更新間隔（デフォルト: 100件ごと）
        """
        self.total = total
        self.desc = desc
        self.current = 0
    ...
    def update(self, n: int = 1):
        ...
        # 更新間隔ごと、または最後のサンプル、または10秒経過時に表示
        should_log = (
            self.current % self.update_interval == 0 or 
            self.current == self.total or
            (datetime.now() - self.last_log_time).total_seconds() > 10
        )
