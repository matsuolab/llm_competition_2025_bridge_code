...
# チーム手順書記載のスクリプトディレクトリを設定
SCRIPT_ROOT="$HOME/team_suzuki/train"
...
for node in "${NODES[@]}"; do
    ...
    ssh -q $node "
        cd $SCRIPT_ROOT && \
        bash $SCRIPT_ROOT/scripts/mutinode_sft/sft_Qwen3-235B-A22B-Thinking-2507.sh $MASTER_ADDR $MASTER_PORT $NODE_RANK $NNODES $GPUS_PER_NODE $DATASET_PATH $CONDA_PATH
